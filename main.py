...
def fake_person_female():
  response = requests.get("https://fakerapi.it/api/v1/persons?_quantity=1&_locale=en_US&_gender=female")
  json_data = json.loads(response.text)
...
def fake_person_male():
  response = requests.get("https://fakerapi.it/api/v1/persons?_quantity=1&_locale=en_US&_gender=male")
  json_data = json.loads(response.text)
  firstname = json_data["data"][0]['firstname']
  lastname = json_data["data"][0]['lastname']
  name = firstname + " " + lastname
...
def fake_credit_Card():
  response = requests.get("https://fakerapi.it/api/v1/credit_cards?_quantity=1")
  json_data = json.loads(response.text)
...
def next_marvel_movie():
  ...
  return (title_1, days_until_1, release_date_1, type_1, overview_1, poster_url_1)
...
async def on_message(message):
  ...
  if msg == '$upcoming-marvel':
    ...
    output_1 = (title_1 + " releases in " + str(days_until_1) + " days!\n\nRelease Date: " + release_date_1 + "\n\nProduction Type: " + type_1 + "\n\nOverview: " + overview_1 + "\n")
    output_2 = (title_2 + " releases in " + str(days_until_2) + " days!\n\nRelease Date: " + release_date_2 + "\n\nProduction Type: " + type_2 + "\n\nOverview: " + overview_2 + "\n")
    red_embed_1 = discord.Embed(color=0xff0000, title="When is the next Marvel Cinematic Universe (MCU) movie/tv show?", description=output_1)
    red_embed_1.set_thumbnail(url=poster_url_1)
    red_embed_1.set_footer(text="Source (Not my website): https://whenisthenextmcufilm.com/")
    await message.channel.send(embed=red_embed_1)
    red_embed_2 = discord.Embed(color=0xff0000, title="What's afterwards? you might ask...", description=output_2)
    red_embed_2.set_thumbnail(url=poster_url_2)
    red_embed_2.set_footer(text="Source (Not my website): https://whenisthenextmcufilm.com/")
